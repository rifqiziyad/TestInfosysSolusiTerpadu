...
interface OurTeamsListProperty {
  src: string;
  name: string;
  posiiton: string;
}
...
type OurTeamListState = {
  lists: OurTeamsListProperty[];
};

const initialState: OurTeamListState = {
  lists: [],
};
...
export const OurTeamsList = createSlice({
  name: "lists",
  initialState,
  reducers: {
    OurTeamsListData: (state) => {
      setTimeout(() => {
        return state;
      }, 3000);
    },
...
export default OurTeamsList.reducer;
export const { OurTeamsListData } = OurTeamsList.actions;
